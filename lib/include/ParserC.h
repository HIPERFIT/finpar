...
/// Writing Dataset ///
///////////////////////
void write_scal( FILE* out, const int   * i ) {
    fprintf(out, "%d ", *i);
}
void write_scal( FILE* out, const double* r ) {
    fprintf(out, "%lf ", *r);
}
void write_scal( FILE* out, const float * r ) {
    fprintf(out, "%f ", *r);
}

template<class T>
void write_1Darr( FILE* out, const T* ptr, const int& N ) {
    fprintf(out, "\n [ ");
    for( int i = 0; i < N-1; i ++ ) {
        write_scal(out, &ptr[i]);
        fprintf(out, ", ");
    }
    write_scal(out, &ptr[N-1]);
    fprintf(out, " ]");
}
