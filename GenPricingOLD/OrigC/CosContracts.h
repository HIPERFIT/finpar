...
static void trajectory_inner(
	...
	index += cur_iter*prod;
#endif
	UINT index = ro_scal->num_contracts*ro_scal->mc_bigit_num*model_num +
					contract_number*ro_scal->mc_bigit_num + cur_iter;
	UINT ind_coef = ro_scal->pc_discounts_beg + model_num*ro_scal->num_cash_flows+date_index;
	priv_arr->model_vhat[index] += (amount * ro_arr->pc_coefs[ind_coef]);

	//instance *inst = (instance*) model->model_data;
	//inst->vhat[contract_number] += amount * inst->discounts[date_index];
}

static void trajectory_contract1( // SIMPLE
		UINT                      model_num,
		UINT                      cur_iter,
		LoopROScalars*            ro_scal,
		struct LoopROArrays*      ro_arr,
	...
	// priv_arr->inst_trajWF[inst_num][md_dim*md_nb_path_dates]
	REAL* inst_traj = priv_arr->inst_trajWF + model_num * (ro_scal->md_dim * ro_scal->md_nb_path_dates);

	REAL amount = fmax(0.0, ((underlyings(0,0)) - 4000.0) *
			ro_arr->pc_coefs[ro_scal->pc_deter_val_beg + model_num*ro_scal->num_det_pricers + 0]);
	//REAL amount = fmax(0., ((underlyings[0][0] - 4000.) * model->deterministic_values[0]))
	//model->notify_cash_flow(model, 0, amount, 0 /*2012-11-30, 2012-11-30, EUR*/);

	trajectory_inner( model_num, cur_iter, ro_scal, ro_arr, priv_arr, 0, amount, 0 );
}

static void trajectory_contract3( // WORST OFF
		UINT                      model_num,
		UINT                      cur_iter,
		LoopROScalars*            ro_scal,
		struct LoopROArrays*      ro_arr,
	...
	trajectory_inner( model_num, cur_iter, ro_scal, ro_arr, priv_arr, 0, 1000., 4 );
	...
	trajectory_inner( model_num, cur_iter, ro_scal, ro_arr, priv_arr, 0, (-(1000. * (1. - x50))), 4 ); return;
L25:
	//model->notify_cash_flow(model, 0, 750., 4 /*2017-02-03, 2017-01-27, EUR*/); return;
	trajectory_inner( model_num, cur_iter, ro_scal, ro_arr, priv_arr, 0, 750., 4 ); return;
L24:
	//model->notify_cash_flow(model, 0, 1600., 3 /*2016-02-03, 2016-01-27, EUR*/); return;
	trajectory_inner( model_num, cur_iter, ro_scal, ro_arr, priv_arr, 0, 1600., 3 ); return;
L23:
	//model->notify_cash_flow(model, 0, 1450., 2 /*2015-02-03, 2015-01-27, EUR*/); return;
	trajectory_inner( model_num, cur_iter, ro_scal, ro_arr, priv_arr, 0, 1450., 2 ); return;
L22:
	//model->notify_cash_flow(model, 0, 1300., 1 /*2014-02-03, 2014-01-27, EUR*/); return;
	trajectory_inner( model_num, cur_iter, ro_scal, ro_arr, priv_arr, 0, 1300., 1 ); return;
L21:
	//model->notify_cash_flow(model, 0, 1150., 0 /*2013-02-01, 2013-01-27, EUR*/); return;
	trajectory_inner( model_num, cur_iter, ro_scal, ro_arr, priv_arr, 0, 1150., 0 ); return;
}
...
static void trajectory_contract2( // BARRIER
		UINT                      model_num,
		UINT                      cur_iter,
		LoopROScalars*            ro_scal,
		struct LoopROArrays*      ro_arr,
...
L48:
	...
	trajectory_inner( model_num, cur_iter, ro_scal, ro_arr, priv_arr, 0, 100., 0 );
	...
	trajectory_inner( model_num, cur_iter, ro_scal, ro_arr, priv_arr, 0, 1000., 1 ); return;
