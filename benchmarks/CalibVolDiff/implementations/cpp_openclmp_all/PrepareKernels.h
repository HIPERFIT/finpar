...
struct NordeaArrays {
    /* RO arrays */
    real_t* timeline; // [NUM_T]

    real_t* myX;   // [NUM_X]
    real_t* myDx;  // [NUM_X * 3]
    real_t* myDxx; // [NUM_X * 3]

    real_t* myY;   // [NUM_Y]
    real_t* myDy;  // [NUM_Y * 3]
    real_t* myDyy; // [NUM_Y * 3]

    /* TRIDAG helpers are created on GPU only! no need for them to exist in main memory*/
    real_t* a;   // [OUTER_LOOP_COUNT * NUM_X * NUM_Y]
    real_t* b;   // [OUTER_LOOP_COUNT * NUM_X * NUM_Y]
    real_t* c;   // [OUTER_LOOP_COUNT * NUM_X * NUM_Y]
    real_t* y;   // [OUTER_LOOP_COUNT * NUM_X * NUM_Y]
    real_t* u;   // [OUTER_LOOP_COUNT * NUM_X * NUM_Y]
    real_t* v;   // [OUTER_LOOP_COUNT * NUM_X * NUM_Y]
    real_t* tmp; // [4 * OUTER_LOOP_COUNT * NUM_X * NUM_Y]

    real_t* res_arr; // [OUTER_LOOP_COUNT * NUM_X * NUM_Y]
...
cl_kernel make_prepare_tridag_X(
    ...
    { // 4. LOCAL (PRIVATE ARRAYS)
        const int PRIV_MULT = sizeof(real_t) * localWorkSize[0] * localWorkSize[1] * localWorkSize[2];
...
cl_kernel make_NordeaKernelX(
    ...
    // 4. LOCAL (PRIVATE ARRAYS)
    const int PRIV_MULT = sizeof(real_t) * 8 * WORKGROUP_SIZE;
...
cl_kernel make_MatMultScan(
    ...
    { // 2. LOCAL: 8 * sizeof(real_t) * worksize
        const int PRIV_MULT = sizeof(real_t) * 8 * localWorkSize;
...
cl_kernel make_FwdFunCompScan(
    ...
    // 2. LOCAL: 4 * sizeof(real_t) * worksize
    const int PRIV_MULT = sizeof(real_t) * 4 * localWorkSize;
...
//        size_t*             localWorkSize
    ...
    // shared memory space
    ciErr1 |= clSetKernelArg(ckMatTransp, counter++, (BLOCK_DIM + 1) * BLOCK_DIM * sizeof(real_t), 0 );
...
void testMatMultScan (
    ...
    real_t* tmp = new real_t[NUM_X*NUM_Y*4];

    for(int i=0; i<NUM_X*NUM_Y*4; i+=4) {
        real_t val = (real_t)(i/4 + 1);
        tmp[i]   = val;   //1.0 + val;
        tmp[i+1] = val+1; //0.0;
    ...
    { // 1. enqueue buffer
        unsigned long int cur_size = 4 * NUM_X * NUM_Y * sizeof(real_t);
    ...
    { // 2. make kernel and execute
        ...
        { // read back from GPU
            ciErr = clEnqueueReadBuffer(
                            ...
                            4 * NUM_X * NUM_Y * sizeof(real_t), tmp, 0, NULL, NULL
                    );
            ciErr |= clFinish(cqCommandQueue);
            oclCheckError(ciErr, CL_SUCCESS);
    ...
    for(int j=0; j<NUM_Y; j++) {
        ...
        real_t* tmpj = tmp + j*NUM_X*4;
        for(int i=0; i<NUM_X; i++) {
            real_t* tmpji = tmpj + i*4;
...
void testFunCompScan (
    ...
    unsigned long int ARR_SIZE = 2 * NUM_X * NUM_Y * sizeof(real_t);

    cl_mem cl_tmp; cl_int ciErr;
    real_t* tmp = new real_t[NUM_X*NUM_Y*4];

    for(int i=0; i<NUM_X*NUM_Y*4; i+=2) {
        real_t val = (real_t)1.0/(i/4 + 1);
        tmp[i]   = val;   //1.0 + val;
        tmp[i+1] = val+1; //0.0;
    }
    ...
    for(int j=0; j<NUM_Y; j++) {
        shrLog(stdlog, "ROW NUM %d:\n\t", j);
        real_t* tmpj = tmp + j*NUM_X*2;
        for(int i=0; i<NUM_X; i++) {
            real_t* tmpji = tmpj + i*2;
