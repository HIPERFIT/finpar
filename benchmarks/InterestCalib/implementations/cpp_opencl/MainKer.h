...
//            ciErr1 |= clSetKernelArg( kernel, counter++, sizeof(uint),          &POP_SIZE                );
        ...
        void copy_out_buffers1() {
            ...
            cur_size = NUM_SWAP_QUOTES * POP_SIZE * sizeof(real_t); // need only to write back the quotes (not the prices)
        ...
        void copy_out_buffers3() {
            ...
            cur_size =      POP_SIZE * sizeof(real_t);
            offset   = 11 * POP_SIZE * sizeof(real_t); 
...
//            cur_size = 2 * NUM_SWAP_QUOTES * POP_SIZE * sizeof(real_t); // need only to write back the quotes (not the prices)
//            ciErr |= clEnqueueReadBuffer(cmd_queue, gpu_buffs.new_quote_price, CL_TRUE, 0, cur_size, cpu_arrs.new_quote_price, 0, NULL, NULL);
        }
};
