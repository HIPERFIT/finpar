...
inline
real_t days_between(real_t t1, real_t t2) {
    return ( (t1 - t2) / MINUTES_IN_DAY ); 
}

inline
real_t date_act_365(real_t t1, real_t t2) { 
    return ( days_between(t1, t2) / 365.0F ); 
}
...
inline
real_t add_months ( real_t time, real_t rnbmonths ) {
    ...
    return (real_t) ( date_of_gregorian( date ) );
}

inline
real_t add_years( real_t date, real_t nbyears ) {
    return add_months( date, nbyears * 12.0F );
}
