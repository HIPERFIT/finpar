...
inline real_t maxR(real_t a, real_t b) { return ( (a <= b) ? b : a ); }
...
void eval_genome_main (
    ...
    __global real_t  *SwaptionQuotes,
    __global real_t  *genomes,
    __global real_t  *glb_arrs,          // ci ++ t1_cs ++ scale ++ bbi
    __global real_t  *new_quote_price,   // [POP_SIZE * NUM_SWAP_QUOTES * 2]
    __global real_t  *interm_scalars,    // [POP_SIZE * NUM_SWAP_QUOTES * 8]
    ...
    __local  real4_t  sh_mem4    [ LWG_EG + (LWG_EG >> 1) ];

    bool iddle = fillShapeMeta( glb_shape, shape_meta, Nshp );

    real_t swap_freq = 1.0, maturity = 1.0, tmat0 = 1.0; uint n_schedi;
    if( !iddle ) {
    ...
    real_t strike, beg_date, end_date;
    { // BLACK PRICE COMPUTATION.
        __local uchar* flags   = (__local uchar*) shape_meta;

        tmp4 = (real4_t) (0.0, MAX_DATE, MIN_DATE, 1.0);
        if(!iddle) {
            tmp4.w  = (real_t) getIotaInd(shape_meta);            // i
...
//
            {
                real_t v0_mat, v0_end, vt_end, baii, bbii, tmp;
...
//
            {
                ...
                real_t  muy, rhoxy, sigmax, sigmay, rhoxyc, rhoxycs, zc_mat, tmp1, tmp2;
...
//
                tmp1   =  sqrt( (real_t)b_fun(2.0*a, tmat0) );   // sqrt_bfun_a
                sigmax =  sigma * tmp1;
                if(is_first) interm_scalars[ gind ] = sqrt((real_t)2.0) * sigmax; gind += offset;
//
                tmp2   =  sqrt( (real_t)b_fun(2.0*b, tmat0) );   // sqrt_bfun_b
                sigmay =  nu    * tmp2;
                rhoxy  =  rho * b_fun(a+b, tmat0) / (tmp1 * tmp2);
                t4     = (rhoxy * sigmay) / sigmax;
//
                rhoxyc  = 1.0 - rhoxy * rhoxy;  // used in reduction kernel
                rhoxycs = sqrt( (real_t)rhoxyc );       // used in reduction kernel
...
void eval_genome_red1 (
    __global   short *glb_shape,
    __constant real_t  *gauss_coefs,    // [Gauss_DIM]
    __constant real_t  *gauss_weights,  // [Gauss_DIM]

    __global real_t  *glb_arrs,          // ci ++ t1_cs ++ scale ++ bbi
    __global real_t  *interm_scalars,    // [POP_SIZE * NUM_SWAP_QUOTES * 8]
                                       // { mux, muy, sqrt_sigmax = sqrt((real_t)2.0) * sigmax,
                                       //   t2 = rhoxy / (sigmax*rhoxycs), sigmay_rhoxycs, zc_mat, f, df }

    __global real_t  *accum0,            // [ POP_SIZE * NUM_SWAP_QUOTES * Gauss_DIM ]

    uint           Nswap,
    uint           Nshp
) {
    __local  short  shape_meta [ LWG_EG << 2 ];
    __local  real_t   sh_mem     [ LWG_EG ];
    ...
    real_t x_quad = gauss_coefs  [get_global_id(1)];

    real_t sigmay_rhoxycs, x, h1;
    if(!iddle){
        real_t mux, muy, sqrt2sigmax, t2, f, df, yhat_x;  // Npop == POP_SIZE = get_global_size(2)
    ...
    if(!iddle){
        uint gind  = get_global_id(2) * Nshp + get_global_id(0), offset = Nshp * get_global_size(2);
             gind += 3 * offset;
        real_t bbi   = glb_arrs[gind]; gind -= offset;
        real_t h2    = h1 + bbi * sigmay_rhoxycs;

        real_t scale = glb_arrs[gind]; gind -= offset;
        real_t t1_cs = glb_arrs[gind]; gind -= offset;
        real_t expo_aici = t1_cs + scale*x;

        real_t expo_part = uGaussian_P_withExpFactor( -h2, expo_aici );
    ...
    { // segmented reduce with operator plus the x's
        ...
        if( is_last ) {
            real_t accum = sh_mem[TH_ID];
            real_t tmp = sqrt((real_t)2.0) * x_quad; //(x - mux) / sigmax;
                 tmp = exp( (real_t)- 0.5 * tmp * tmp );

            real_t w_quad = gauss_weights[get_global_id(1)];

            real_t res    = w_quad * tmp * ( uGaussian_P(-h1) - accum );
...
/********************************************************/
inline real_t cauchy_pdf( real_t z, real_t mu, real_t gamma ) {
    real_t x = (z-mu) / gamma;
    return 1.0 / ( PI * gamma * (1.0+x*x) );
}
inline real_t logLikelihood_cauchy( real_t y_ref, real_t y ) {
    real_t LLHOOD_CAUCHY_OFFS = 5.0;
...
void eval_genome_red2 (
    ...
    if (!iddle) {
        ...
        real_t zc_mat = interm_scalars[ gid + (5 * get_global_size(1) * Nswap) ];

        real_t nprice = zc_mat * ( accum / sqrt( PI ) );
    ...
    real_t rms = reduce_reg_plus( sh_mem );
