...
void makeSummary(   const int     winner,
                    const Genome& wgene,
                    const real_t*   new_quote,
                    const real_t*   new_price,
                    real_t*   result // output
                    ) {
  real_t rms = 0.0;
  ...
  for( int i = 0; i < NUM_SWAP_QUOTES; i ++ ) {
    real_t black_price = new_quote[i];
    real_t calib_price = new_price[i];
 ...
 *    of the best genome.
 */
void find_best(const Genome* genomes, int& best_ind, real_t& best_lik) {
  bool sanity = true;
 ...
 */
real_t* mainKernelSeqCPU( Genome& winner ) {
  ...
  for( int j = 0; j < MCMC_LOOPS; j++ ) {
    ...
    for ( int i = 0; i < POP_SIZE; i++ ) {  // parallel
      ...
      real_t acceptance = std::min( 1.0, exp( muta.logLik - orig.logLik ) * muta.fbRat );
  ...
  real_t* result = new real_t[3*NUM_SWAP_QUOTES];
  { // print best candidate for the current iteration:
    int best_ind; real_t best_lik;
    find_best(genomes, best_ind, best_lik);
    ...
    real_t* quotes = cpu_arrs.get_quote();
    real_t* prices = cpu_arrs.get_price();
    for( UINT ttt = 0; ttt < NUM_SWAP_QUOTES; ttt++ ) {
      eval_genome_new (
