...
struct oclLoopArrays {
    ...
    cl_mem  bb_data;        // real_t[3][num_dates]

    /* MD instance data; RO */       
    cl_mem  md_c;             // Includes all arrays bellow:
//    cl_mem  md_c            // RO; real_t[num_models][num_under^2]
//    cl_mem  md_vols;        // RO; real_t[num_models][num_under*num_dates]
//    cl_mem  md_drifts;      // RO; real_t[num_models][num_under*num_dates]
//    cl_mem  md_starts;      // RO; real_t[num_models][num_under]
//    cl_mem  md_discts;      // real_t[num_models][num_cash_flows ]
//    cl_mem  md_detvals;     // real_t[num_models][num_det_pricers]

    /* FOR VECTORIZED VERSION */
    cl_mem  md_zd;    // RW: real_t[num_gpuits]x[num_under*num_dates]
    cl_mem  md_z;  // RW: real_t[num_gpuits]x[num_models]x[num_under*num_dates]
...
oclAllocArrays_PrivKernel (
    ...
    { // 3. BROWNIAN BRIDGE RO indirect arrays and data arrays
        ...
        cur_size = 3*ro_scals.num_dates*sizeof(real_t);
    ...
    { // 4. Various RO Model Data Arrays
        ...
        cur_size *= ro_scals.num_models * sizeof(real_t);
    ...
    { // 5. Finally, the WO Partial Result
        ...
        glb_vhat = (real_t*)malloc(cur_size);
...
runGPU_PRIV(
    ...
    const int PRIV_MULT   = sizeof(real_t)*ro_scals.BLOCK;
    ...
    priv_sz = (globalWorkSize[0]/ro_scals.BLOCK)*ro_scals.num_models*sizeof(real_t);
...
void reduceVHAT_CPU(const LoopROScalars& ro_scals, real_t* vhat, double* cpu_vhat ) {
...
void runGPU_VECT(
    ...
    { // compute trajectory
        ...
        if( optim_kernel ) {
            ...
            UINT priv_sz = TILE * ro_scals.BLOCK * sizeof(real_t);
    ...
    { // final reduction!
        ...
        ciErr1 |= clEnqueueReadBuffer(  cqCommandQueue, dev_arr.model_vhat, CL_TRUE, 0,
                                        globalWorkSize[0] * ro_scals.num_models * sizeof(real_t),
