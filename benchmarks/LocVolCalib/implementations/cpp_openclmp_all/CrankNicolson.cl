...
inline void store2(real2_t val, unsigned int ind,  __local real_t* cache) {
...
inline real2_t load2(unsigned int ind,  __local real_t* cache) {
    ind = (ind & (WORKGROUP_SIZE-1)) + ( (ind >> logWORKGROUP_SIZE) << (logWORKGROUP_SIZE+1) );
    real2_t val;
    val.x = cache[ind];
    ind  += WORKGROUP_SIZE;
...
#else
inline void store2(real2_t val, unsigned int ind, volatile __local real2_t* cache) {
    cache[ind] = val;
}

inline real2_t load2(unsigned int ind, volatile __local real2_t* cache) {
    return cache[ind];
}
#endif
...
inline void store4(real4_t val, unsigned int ind,  __local real_t* cache) {
...
inline real4_t load4(unsigned int ind,  __local real_t* cache) {
    real4_t val;
    //ind = (ind / WORKGROUP_SIZE)*(WORKGROUP_SIZE) * 4 + (ind % WORKGROUP_SIZE);
    ind = (ind & (WORKGROUP_SIZE-1)) + ( (ind >> logWORKGROUP_SIZE) << (logWORKGROUP_SIZE+2) );
    val.x = cache[ind];
...
#else
inline void store4(real4_t val, unsigned int ind, volatile __local real4_t* cache) {
    cache[ind] = val;
}

inline real4_t load4(unsigned int ind, volatile __local real4_t* cache) {
    return cache[ind];
}
#endif
/**************************************************************************/
...
__kernel void prepare_tridag_x (
        ...
        /*** Read-Only Arrays  ***/
        __global real_t*         myX,
        __global real3_t*        myDx,
        __global real3_t*        myDxx,
        __constant real_t*         myY,
        ...
        __global   real_t*        u,
        __global   real_t*        v,
        __global   real4_t*       scan_tmp,
        /*** Temporary Array in Shared Space ***/   
        __local    real_t*        cache_tmp,
        /*** The Result Array ***/
        __global   real_t*        res_arr        
) {
    real3_t myDy_elem, myDyy_elem, myres_elem;
    real_t cur_myVar = ro_scals->nu; 
    real_t cur_myMu  = 0.0;
    ...
    // second loop    
    {
        myres_elem = (real3_t)(   (get_global_id(1)!=0) ? res_arr[ind - get_global_size(0)] : 0.0, 
                                res_arr[ind], 
                                (get_global_id(1) != get_global_size(1)-1) ? res_arr[ind + get_global_size(0)] : 0.0
                            );
 
        myDy_elem  = (real3_t)( myDy[get_global_id(1)<<2], myDy[ (get_global_id(1)<<2)+1], myDy[(get_global_id(1)<<2)+2] );
        myDyy_elem = (real3_t)( myDyy[get_global_id(1)<<2], myDyy[(get_global_id(1)<<2)+1], myDyy[(get_global_id(1)<<2)+2] );
    ...
    { 
        ...
        myres_elem = (real3_t)(     (get_global_id(0)!=0) ? cache_tmp[ glob_ind - 1 ] : 0.0, 
                                  cache_tmp[ glob_ind ], 
    ...
    if( get_global_id(0) > 0 ) { 
        scan_tmp[ind] = (real4_t) ( tmp, myDy_elem.x * c[ind-1], 1.0, 0.0 );
    } else { // i == get_global_id(0) == 0
        y[ind] = tmp;    // = b [ind]
        scan_tmp[ind] = (real4_t) ( tmp, 0.0, 0.0, tmp ); 
    }
} 
...
__kernel void prepare_tridag_y (
        ...
        __global RWScalars*   ro_scals,  // __constant
        /*** Read-Only Arrays  ***/
        __global real3_t*        myDy,
        __global real3_t*        myDyy,
        /*** Read-Write Temporary Arrays ***/
        __global   real_t*        a,
        __global   real_t*        b,
        __global   real_t*        c,
    ...
    real3_t myDy_elem;
    real_t dt_inv = ro_scals->dtInv;

    real_t cur_myMuY = 0.0;
    real_t cur_myVarY = ro_scals->nu; cur_myVarY *= cur_myVarY*0.5f; //nu*nu;
    ...
    if( get_global_id(0) > 0 ) { 
#ifdef USE_LOCAL_Y
        scan_tmp[ind] = (real4_t) ( dt_inv - myDy_elem.y, myDy_elem.x * cache_tmp[glob_ind-1], 1.0, 0.0 );
#else
        const real_t elem = - 0.5f*(cur_myMuY*myDy[get_global_id(0)-1].z + cur_myVarY*myDyy[get_global_id(0)-1].z);
        scan_tmp[ind] = (real4_t) ( dt_inv - myDy_elem.y, myDy_elem.x * elem, 1.0, 0.0 );   
#endif
        //scan_tmp[ind] = (real4_t) ( dt_inv - myDy_elem.y, myDy_elem.x*c[ind-1], 1.0, 0.0 );

    } else { // j == get_global_id(0) == 0
        y[ind] = dt_inv - myDy_elem.y; //b[ind];
        scan_tmp[ind] = (real4_t) ( dt_inv - myDy_elem.y, 0.0, 0.0, dt_inv - myDy_elem.y );
    }
}
 ...
 */ 
inline real4_t matmult2(real4_t a, real4_t b) {
    real_t val = (a.x*b.x + a.y*b.z);

    a = (real4_t) (val, a.x*b.y + a.y*b.w, a.z*b.x + a.w*b.z, a.z*b.y + a.w*b.w);
    val = 1.0/val;
    return a * val;
}
...
#if WITH_INTERLEAVED_BANKS4
inline real4_t warpScanInclMatMult(real4_t idata, volatile __local real4_t *l_Data4, uint size){
    volatile __local real_t* l_Data = (volatile __local real_t*) l_Data4;

    uint pos = 2 * get_local_id(0) - (get_local_id(0) & (size - 1));
    store4( (real4_t)(1.0, 0.0, 0.0, 1.0), pos, l_Data );
    pos += size;
    store4( idata, pos, l_Data );  

    if(size >=  2) { real4_t tmp = matmult2( load4(pos, l_Data),load4(pos-1, l_Data) ); store4(tmp, pos, l_Data); }
    if(size >=  4) { real4_t tmp = matmult2( load4(pos, l_Data),load4(pos-2, l_Data) ); store4(tmp, pos, l_Data); }
    if(size >=  8) { real4_t tmp = matmult2( load4(pos, l_Data),load4(pos-4, l_Data) ); store4(tmp, pos, l_Data); }
    if(size >= 16) { real4_t tmp = matmult2( load4(pos, l_Data),load4(pos-8, l_Data) ); store4(tmp, pos, l_Data); }
#if WARP == 32
    if(size >= 32) { real4_t tmp = matmult2( load4(pos, l_Data),load4(pos-16,l_Data) ); store4(tmp, pos, l_Data); }
#endif
    return load4(pos, l_Data);
}
#else
inline real4_t warpScanInclMatMult(real4_t idata, volatile __local real4_t *l_Data, uint size){
    uint pos = 2 * get_local_id(0) - (get_local_id(0) & (size - 1));
    l_Data[pos] = (real4_t)(1.0, 0.0, 0.0, 1.0); 
    pos += size;
    l_Data[pos] = idata;  
...
inline real4_t scanMatMultInclLocal(real4_t idata4, __local real4_t *l_Data, uint size){
    if(size > WARP){
        //Bottom-level inclusive warp scan
        real4_t warpResult = warpScanInclMatMult(idata4, l_Data, WARP);
...
#if WITH_INTERLEAVED_BANKS4
            store4(warpResult, get_local_id(0) >> lgWARP, (__local real_t*)l_Data);
#else
...
#if WITH_INTERLEAVED_BANKS4
            real4_t val = load4(get_local_id(0), (__local real_t*)l_Data);    
            store4( warpScanInclMatMult(val, l_Data, size >> lgWARP), get_local_id(0), (__local real_t*)l_Data);
#else
            //grab top warp elements
            real4_t val = l_Data[get_local_id(0)];
...
#if WITH_INTERLEAVED_BANKS4
            real4_t tmp = load4( (get_local_id(0) >> lgWARP) - 1,   (__local real_t*)l_Data );   
...
void scanMatMultIncl(
    __global real4_t *data,
    __local  real4_t *l_Data,
    uint size
){
    //Load data
    real4_t idata4 = data[get_global_id(0)];

    //Calculate exclusive scan
    real4_t odata4  = scanMatMultInclLocal( idata4, l_Data, size );
...
inline real2_t linfuncomp(real2_t ab2, real2_t ab1) {
...
#if WITH_INTERLEAVED_BANKS2
inline real2_t warpScanInclLinFunComp(real2_t idata, volatile __local real2_t *l_Data2, uint size){
    volatile __local real_t* l_Data = (volatile __local real_t*) l_Data2;

    uint pos = 2 * get_local_id(0) - (get_local_id(0) & (size - 1));
    store2( (real2_t)(0.0, 1.0), pos, l_Data );
    pos += size;
    store2( idata, pos, l_Data );  

    if(size >=  2) { real2_t tmp = linfuncomp( load2(pos, l_Data),load2(pos-1, l_Data) ); store2(tmp, pos, l_Data); }
    if(size >=  4) { real2_t tmp = linfuncomp( load2(pos, l_Data),load2(pos-2, l_Data) ); store2(tmp, pos, l_Data); }
    if(size >=  8) { real2_t tmp = linfuncomp( load2(pos, l_Data),load2(pos-4, l_Data) ); store2(tmp, pos, l_Data); }
    if(size >= 16) { real2_t tmp = linfuncomp( load2(pos, l_Data),load2(pos-8, l_Data) ); store2(tmp, pos, l_Data); }
#if WARP == 32
    if(size >= 32) { real2_t tmp = linfuncomp( load2(pos, l_Data),load2(pos-16,l_Data) ); store2(tmp, pos, l_Data); }
#endif
    return load2(pos, l_Data);
}
#else
inline real2_t warpScanInclLinFunComp(real2_t idata, volatile __local real2_t *l_Data, uint size){
    uint pos = 2 * get_local_id(0) - (get_local_id(0) & (size - 1));
    l_Data[pos] = (real2_t)(0.0, 1.0);
    pos += size;
    l_Data[pos] = idata;
...
inline real2_t scanLinFunCompInclLocal(real2_t idata4, __local real2_t *l_Data, uint size){
    if(size > WARP){
        //Bottom-level inclusive warp scan
        real2_t warpResult = warpScanInclLinFunComp(idata4, l_Data, WARP);
        ...
        if( (get_local_id(0) & (WARP - 1)) == (WARP - 1) ) { 
#if WITH_INTERLEAVED_BANKS2
            store2(warpResult, get_local_id(0) >> lgWARP, (__local real_t*)l_Data);
#else
            l_Data[get_local_id(0) >> lgWARP] = warpResult;
#endif
        }
...
#if WITH_INTERLEAVED_BANKS2
            real2_t val = load2(get_local_id(0), (__local real_t*)l_Data);    
            store2( warpScanInclLinFunComp(val, l_Data, size >> lgWARP), get_local_id(0), (__local real_t*)l_Data);
#else
            //grab top warp elements
            real2_t val = l_Data[get_local_id(0)];
            //calculate inclusive scan and write back to shared memory
        ...
        if( ( (get_local_id(0) & (size-1)) >> lgWARP ) > 0 ) {
#if WITH_INTERLEAVED_BANKS2
            real2_t tmp = load2( (get_local_id(0) >> lgWARP) - 1,   (__local real_t*)l_Data );   
...
void scanLinFunCompIncl(
    __global real2_t *data,
    __local  real2_t *l_Data,
    uint size
){
    //Load data
    real2_t idata4 = data[get_global_id(0)];

    //Calculate exclusive scan
    real2_t odata4  = scanLinFunCompInclLocal( idata4, l_Data, size );
...
inline real_t map_matmult(real4_t tmp, real_t val1) {

    real_t nom   = tmp.x*val1+tmp.y;
    real_t denom = tmp.z*val1+tmp.w;

    return nom/denom;

    //real_t denom = tmp.z*val1+tmp.w*val2;
    //real_t nom   = (tmp.x/denom)*val1 + (tmp.y/denom)*val2;
    //return nom;
}
...
__kernel void prelude_fwd_fun_comp (
        ...
        __global   real_t*        d,
        __global   real_t*        u,
        __global   real2_t*       scan_tmp
) {
    const unsigned int ind = get_global_id(2) * get_global_size(1) * get_global_size(0)  
                                + get_global_id(1) * get_global_size(0) + get_global_id(0);

    if( get_global_id(0) > 0 ) { 
        scan_tmp[ind] = (real2_t) ( d[ind], (0.0 - (a[ind]/u[ind-1])) );
    } else {
        scan_tmp[ind] = (real2_t) (0.0, 1.0);
    }
}
...
__kernel void prelude_bwd_fun_comp (
    ...
    if( get_global_id(0) <  get_global_size(0) - 1 ) {
        scan_tmp[ind] = (real2_t)(y[inv_ind]/u[inv_ind], 0.0 - c[inv_ind]/u[inv_ind]);
...
__kernel void post_bwd_fun_comp (
    ...
    real_t ynm1 = y[inv_ind + get_global_id(0) + 1];
...
//        __global   real_t*         d,
        real_t                    d_elem,
        __global   real_t*        y,
//        __global   real4_t*       scan_tmp4,
        real4_t                   data4,
        const unsigned int      SIZE,
/*** Temporary Array in Shared Space ***/   
        __local    real_t*        cache_tmp,   // size: [8*WORKSIZE*sizeof(real_t)]
    ...
    { // Forward scan with linear function composition
        ...
        if(get_global_id(0) == first_id) {
            ...
            data2 = (real2_t) (0.0, 1.0);
        } else {
            data2 = (real2_t) ( d_elem, (0.0 - (a_elem/cache_tmp[get_local_id(0)-1])) );
        }

        {//Calculate exclusive scan
            __local    real2_t*  l_Data = ((__local real2_t*)cache_tmp) + get_local_size(0);
    ...
    { // Backward scan with linear function composition
        ...
        if(get_global_id(0) == first_id) {
            ...
            real_t myu = cache_tmp[local_offset + inv_ind];
            data2 = (real2_t) (   cache_tmp[local_offset + inv_ind + get_local_size(0)] / myu, 
                                0.0 - c_elem / myu );
...
void nordea_kernel_x (
    ...
    { 
        ...
        myres_elem = (real3_t)( myDyy[ind_y<<2], myDyy[(ind_y<<2)+1], myDyy[(ind_y<<2)+2] );
        myD_elem   = (real3_t)( myDy [ind_y<<2], myDy [(ind_y<<2)+1], myDy [(ind_y<<2)+2] );

        myD_elem   = cur_myMu*myD_elem + 0.5f*cur_myVar*myres_elem; 

        myres_elem = (real3_t)(   (ind_y != 0) ? res_arr[get_global_id(0) - ro_scals->NUM_X] : 0.0, 
        ...
        myres_elem = (real3_t)(     (ind_x!=0) ? cache_tmp[ get_local_id(0) - 1 ] : 0.0, 
    ...
    { 
        ...
        if( ind_x > 0 ) { 
            ...
            scan_elem  = (real4_t) ( tmp, 0.0 - myD_elem.x * myD_elem.y, 1.0, 0.0 );
            myD_elem.y = cache_tmp[get_local_size(0) + get_local_id(0) + (ro_scals->NUM_X-1) - 2*ind_x]; // i.e., c[glb_ind_inv]
        } else { // i == get_global_id(0) == 0
            scan_elem = (real4_t) ( tmp, 0.0, 0.0, tmp ); //(real4_t) ( 1.0, 0.0, 0.0, 1.0 );
        ...
        real_t d0 = cache_tmp[get_local_id(0) - ind_x + 1];
...
void nordea_kernel_y (
        ...
        __global   real_t*        v,
        /*** Temporary Array in Shared Space ***/   
        __local    real_t*        cache_tmp    // SIZE: 8 * WORKGROUP_SIZE * sizeof(real_t)
) {
...

    {
        real_t dt_inv = ro_scals->dtInv;
#if TRANSPOSE_UV
        real_t my_u   = u[get_global_id(0)];
#else
        real_t my_u   = u[    (get_global_id(0)/ro_scals->NUM_XY)*ro_scals->NUM_XY + 
...

    {
        ...
        // third loop 
        if( ind_y > 0 ) { 
            scan_elem = (real4_t) ( myDy_elem.y, 0.0 - myDy_elem.x * cache_tmp[get_local_size(0)+get_local_id(0)-1], 1.0, 0.0 );
        } else { 
            scan_elem = (real4_t) ( 1.0, 0.0, 0.0, 1.0 );
        }
