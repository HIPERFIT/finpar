...
void iteration_expanded_CPU (
        ...
        const real_t         beta,
        const real_t         nu,
        real_t*              a, // [OUTER_LOOP_COUNT*NUM_XY]
        real_t*              b, // [OUTER_LOOP_COUNT*NUM_XY]
        real_t*              c, // [OUTER_LOOP_COUNT*NUM_XY]
        real_t*              y, // [OUTER_LOOP_COUNT*NUM_XY]
        real_t*              u, // [OUTER_LOOP_COUNT*NUM_XY]
        real_t*              v, // [OUTER_LOOP_COUNT*NUM_XY]
        real_t*        scan_tmp // [4*OUTER_LOOP_COUNT*NUM_XY]
) {
    unsigned int i, j, k;

    real_t dtInv = 1.0/(myTimeline[time_ind+1]-myTimeline[time_ind]);

    real_t *res_arr = &myResArr[0];
    ...
    for( k=0; k<OUTER_LOOP_COUNT; ++k ) {
        ...
        for(j=0; j<NUM_Y; j++)  {
            for(i=0; i<NUM_X; i++) {
                ...
                unsigned int mul3;
                real_t tmp; //tmp2, tmp1 = dtInv*res_arr[j*NUM_X+i]; //

                real_t cur_myMuX  = 0.0, cur_myMuY = 0.0;
                real_t cur_myVarY = nu*nu;
                real_t cur_myVarX = exp(2*(beta*log(myX[i]) + myY[j] - 0.5*nu*nu*myTimeline[time_ind]));
