...
void run_trimmed_GPUkernels_one_time_iteration (
    ...
    cl_int ciErr;
    const unsigned int ARR_SIZE = NUM_X * NUM_Y * OUTER_LOOP_COUNT * sizeof(real_t);
#if 1
    ...
    real_t dtInv = 1.0/(cpu_arrs.timeline[time_ind+1]-cpu_arrs.timeline[time_ind]);

    real_t *res_arr = cpu_arrs.res_arr, *a = cpu_arrs.a,
