...
void verifyEnoughResources(cl_device_id device) {
    ...
    cl_ulong glob_mem_required = 8 * OUTER_LOOP_COUNT * NUM_X * NUM_Y * sizeof(real_t);
...
void makeOclBuffers (
    ...
    /*********************/
    {
        cur_size = NUM_T*sizeof(real_t);
        CREATE_AND_ENQUEUE_RO_CL_BUFFER(timeline);

        cur_size = NUM_X*sizeof(real_t);
    ...
    /*********************/
    {
        cur_size = OUTER_LOOP_COUNT * NUM_X * NUM_Y * sizeof(real_t);
...
runOnGPU ( RWScalars& ro_scal, NordeaArrays& cpu_arrs, oclNordeaArrays& ocl_arrs ) {
    ...
    unsigned int dev_id = GPU_DEV_ID;
    ...
    { // making command queue, building program, etc
        ...
        sprintf(compile_opts,
                "-D lgWARP=%d -D%s -I%s/include -I%s/include",
                lgWARP, REAL_FLAG, HIPERMARK_BENCHMARK_LIB_DIR, HIPERMARK_LIB_DIR);
