...
#if 0
    unsigned char res[POP_SIZE];
    cl_command_queue& cmd_queue = ocl_objs.getCommandQueue();
    size_t cur_size = POP_SIZE * sizeof(unsigned char);
    cl_int ciErr = clEnqueueWriteBuffer(cmd_queue, gpu_buffs.accept_cond, CL_TRUE, 0, cur_size, res, 0, NULL, NULL);
    printf("Accept[115]: %d %f %f\n", res[115], 0.0, 0.0);
#endif
