...
struct CpuArrays {
    ...
    CpuArrays (const uint n, short* shp) : SS(n) {
        ...
        accum0          = new REAL [ NUM_SWAP_QUOTES * Gauss_DIM * POP_SIZE ];
    ...
    REAL* get_accum (int i) { return accum0 + i * NUM_SWAP_QUOTES * Gauss_DIM; }
};
...
struct OclBuffers {
    ...
    cl_mem    accum0;          // [ NUM_SWAP_QUOTES * Gauss_DIM ]
...
void makeOclBuffers ( CpuArrays& cpu_arrs, OclObjects& ocl_objs, OclBuffers& ocl_arrs ) {
    ...
    //printf("Size of cl_short %d, sizeof short: %d\n\n", sizeof(cl_short), sizeof(short));
    ...
    ciErr |= clEnqueueWriteBuffer(cmd_queue, ocl_arrs.sobol_dir_vct, CL_TRUE, 0, cur_size, SOBOL_DIR_VCT, 0, NULL, NULL);
    ...
    cur_size = Gauss_DIM * sizeof(REAL);
    ocl_arrs.gauss_coefs = clCreateBuffer( ocl_objs.context, CL_MEM_READ_ONLY, cur_size, NULL, &ciErr2 );    ciErr |= ciErr2;
    ciErr |= clEnqueueWriteBuffer(cmd_queue, ocl_arrs.gauss_coefs, CL_TRUE, 0, cur_size, GaussHermiteCoefs::coefs, 0, NULL, NULL);
    ocl_arrs.gauss_weights = clCreateBuffer( ocl_objs.context, CL_MEM_READ_ONLY, cur_size, NULL, &ciErr2 );    ciErr |= ciErr2;
    ciErr |= clEnqueueWriteBuffer(cmd_queue, ocl_arrs.gauss_weights, CL_TRUE, 0, cur_size, GaussHermiteCoefs::weights, 0, NULL, NULL);
...
//    ciErr |= clEnqueueWriteBuffer(cmd_queue, ocl_arrs.scalars, CL_TRUE, 0, cur_size, cpu_arrs.scalars, 0, NULL, NULL);
    ...
    cur_size = NUM_SWAP_QUOTES * Gauss_DIM * POP_SIZE * sizeof(REAL);
