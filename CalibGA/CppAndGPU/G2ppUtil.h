...
#if 0
REAL black_price1(  const REAL& today, 
    ...
    const REAL sqrtt = date_act_365( sos.maturity, today );

    // morally equivalent to `swap_schedule2lvl(swap_schedule)' but in map-reduce form!!

    REAL lvl = 0.0, t0 = MAX_DATE, tn = MIN_DATE; 
    for(UINT i=0; i<sos.n; i++) {
        const REAL a1 = sos.swap_sched1[i];
        const REAL a2 = sos.swap_sched2[i];
    ...
    const REAL s0 = ( zc(t0) - zc(tn) ) / lvl;
    const REAL d1 = log( s0 / sos.strike ) / ( vol * sqrtt ) + 0.5 * vol * sqrtt;
    const REAL d2 = d1 - vol * sqrtt;

    sos.cleanUp();

    return ( lvl * ( s0 * uGaussian_P(d1) - sos.strike * uGaussian_P(d2) ) );
}
#endif
