...
REAL add_months ( const REAL& time, const REAL& rnbmonths ) {
    ...
    date.day = std::min( date.day, end_of_month(date.year, date.month) );
    date.hour = 12;
    date.min  = 0;
...
void test_dates() {
    ...
    printf("add_months(min_date,1)==48240 : ");
    tmp = add_months(MIN_DATE, 1.0);
    if( equalEps(tmp, 48240.0) )   printf("SUCCESS!\n\n");
    else                           printf("res: %f, FAILS!\n\n", tmp);

    printf("add_months(min_date,2)==90000 : ");
    tmp = add_months(MIN_DATE, 2.0);
    if( equalEps(tmp, 90000.0) )   printf("SUCCESS!\n\n");
    else                           printf("res: %f, FAILS!\n\n", tmp);


    printf("add_years(min_date,1)==530640 : ");
    tmp = add_years(MIN_DATE, 1.0);
    if( equalEps(tmp, 530640.0) )  printf("SUCCESS!\n\n");
    else                           printf("res: %f, FAILS!\n\n", tmp);

    printf("add_years(min_date,5)==2634480 : ");
    tmp = add_years(MIN_DATE, 5.0);
    if( equalEps(tmp, 2634480.0) ) printf("SUCCESS!\n\n");
    else                           printf("res: %f, FAILS!\n\n", tmp);
    
    printf("days_between(max_date(),min_date()) == 116877.499306 : ");
    tmp = days_between( MAX_DATE, MIN_DATE ) ;
    if( equalEps(tmp, 116877.499305555) ) printf("SUCCESS!\n\n");
    else                                  printf("res: %f, FAILS!\n\n", tmp);

    printf("act_365(max_date,min_date) == 320.212327 : ");
    tmp = date_act_365( MAX_DATE, MIN_DATE );
    if( equalEps(tmp, 320.2123268645) )   printf("SUCCESS!\n\n");
    else                                  printf("res: %f, FAILS!\n\n", tmp);
}
