...
REAL eval_genome(   const REAL a, 
    ...
    for( UINT ttt = 0; ttt < NUM_SWAP_QUOTES; ttt++ ) {
        const REAL mat_year  = SwaptionQuotes[ttt][0];
        const REAL swap_freq = SwaptionQuotes[ttt][1];
        const REAL term_year = SwaptionQuotes[ttt][2];
        const REAL quote     = SwaptionQuotes[ttt][3];
