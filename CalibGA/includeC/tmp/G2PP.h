...
//
    ...
    for( UINT i = 0; i < n_schedi; i++ ) {
        ...
        if( isinf(aici[i]) || isnan(aici[i]) ) {
            printf("NaN aici in pricer of swaption. Exiting!\n"); exit(0);
        }
    }
//
    ...
    REAL accum0 = 0.0;
    for( UINT j = 0; j < Gauss_DIM; j++ ) {
        const REAL x_quad = GaussHermiteCoefs::coefs  [j];
        const REAL w_quad = GaussHermiteCoefs::weights[j];
...
//            accum1 += fact_aici * exp(expo_aici) * uGaussian_P(-h2);
            ...
            if( isnan(accum1) || isinf(accum1) ) {
                printf( "accum is Nan -- expo_aici: %f, fact_aici: %f, h2: %f, t1_cs: %f, scale: %f, x: %f, bbi: %f, mux: %f, muy: %f\n", 
                        expo_aici, fact_aici, h2, t1_cs[i], scale[i], x, bbi[i], mux, muy );
                printf( "accum is Nan -- t4: %f, rhoxyc: %f, v0mat: %f, b: %f, aici: %f\n", 
    ...
    if( isnan(accum0) || isinf(accum0) ) {
        printf("Numerically Instable Implem: end of pricer_of_swaption: accum0 in NaN or Inf!\n");
        exit(0);
    }
...
REAL pricer (const REAL a = 0.02453, const REAL b = 0.98376, const REAL rho = -0.82400, const REAL nu = 0.11830, const REAL sigma = 0.02398) {
    ...
    for( UINT i = 0; i < NUM_SWAP_QUOTES; i++ ) {
        const REAL mat_year  = SwaptionQuotes[i][0];
        const REAL swap_freq = SwaptionQuotes[i][1];
        const REAL term_year = SwaptionQuotes[i][2];
        const REAL quote     = SwaptionQuotes[i][3];
