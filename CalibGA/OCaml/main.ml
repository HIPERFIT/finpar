...
(*
let () =
  let params={g_a = 0.02453; g_b = 0.98376; g_sigma = 0.02398; g_nu = 0.11830; g_rho = -0.82400} in
  let {g_a; g_b; g_sigma; g_nu; g_rho} = params in
  Printf.printf "a = %.5f, b = %.5f, sigma = %.5f, nu = %.5f, rho = %.5f\n" g_a g_b g_sigma g_nu g_rho;
  let swaption={swaption_maturity_in_year = 10; swap_term_in_year = 4; swap_frequency = Freq_6M} in
  Printf.printf "swaption 10Y4Y: %.3f\n" (10000. *. pricer_of_swaption today zc swaption params);
  let swaption={swaption_maturity_in_year = 30; swap_term_in_year = 30; swap_frequency = Freq_6M} in
  Printf.printf "swaption 30Y30Y: %.3f\n" (10000. *. pricer_of_swaption today zc swaption params);
*)
(**)
let () =
