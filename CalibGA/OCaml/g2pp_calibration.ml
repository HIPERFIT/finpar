...
let pricer_of_swaption ~today ~zc swaption =
  ...
  let bai = Array.create n_schedi 0. in
  let bbi = Array.create n_schedi 0. in
  let aici = Array.create n_schedi 0. in
  let log_aici = Array.create n_schedi 0. in
  let scales = Array.create n_schedi 0. in
  let t1_cst = Array.create n_schedi 0. in
  let scale = Array.create n_schedi 0. in
  fun params ->
    ...
    let exact_yhat x =
      ...
      if n_schedi = 1 then lo
        ...
        if y1 <= yl then y1 +. 1. (* yhat is greater than y1 => 1 - phi(h_i(x, yhat)) < epsilon *)
          ...
          let root=brent root_lb root_ub to_solve 1e-4 in
          (*Printf.printf "# DEVEL: Brent call: (%.3f, %.3f) -> %.3f" root_lb,root_ub,root;*)
          root
        with
